...
public class ListReferenceBased<E> implements ListInterface<E>, Iterable<E> {
    ...
    /** Constructor that takes one generic element as its content */
    public ListReferenceBased(E item){
        add(1, item);
    }

    /**
     * Creates a copy of the passed in class in the passed class
     * contains no elements that it uses the default constructor.
     * @param copy
     */
    public ListReferenceBased(ListReferenceBased<E> copy){
        if (copy.size() > 0) {
            for (int i = 1; i <= copy.size(); i++){
                this.add(i, copy.get(i));
            }
        }
        else {
            new ListReferenceBased();
        }
    }
    ...
    public void add(int index, E item)
        ...
        if (index >= 1 && index <= numItems + 1) {
            ...
            } // end if import java.util.Iterator;
            numItems++;
        } else {
            throw new ListIndexOutOfBoundsException(
    ...
    public void displayReverse() {
        if (numItems != -1) {
            ...
            for (; last != null; last = last.getPrecede()) {
